...
if user_input_origin == "busch student center":
    user_input_origin = 'busch campus center'
user_input_destination = input("Enter your destination address: ")
if user_input_destination == "busch student center":
    user_input_destination = 'busch campus center'

# Display the user input
print("This is your origin address: ", user_input_origin)
print("This is your destination address: ", user_input_destination)

gmaps = googlemaps.Client(key='AIzaSyB2SK10Re26k5hrwUiy44u0RAvTQR6xPfY')

geocode_result = gmaps.geocode(user_input_origin + ', New Brunswick, NJ')
...
print('origin stops ', origin_stops)
    ...
    for prediction in data[bus]['predictions']:
        ...
        if len(origin_stops) > 1:
            if prediction['stationName'] == origin_stops[0] or prediction['stationName'] == origin_stops[1]:
                ...
                set_o.append((prediction['stationName'], ETA))
        else:
            if prediction['stationName'] == closest_o:
                origin_included = True
                set_o.append((closest_o, ETA))
        if len(destination_stops) > 1:
            if prediction['stationName'] == destination_stops[0] or prediction['stationName'] == destination_stops[1]:
                ...
                set_d.append((prediction['stationName'], ETA))
        else:
            if prediction['stationName'] == closest_d or data[bus]['dest'] == closest_d:
                destination_included = True
                set_d.append((closest_d, ETA))
        # print(origin_included, destination_included)
    if (origin_included and destination_included) and set_o[0][1] < set_d[0][1]:
        routes.append((bus, data[bus]['line'], set_o, set_d))
totalTime = None
busToTake = None

if routes:
    earliest_route = min(routes, key=lambda x: x[3][0][1])
    busToTake = earliest_route[0]
    totalTime = earliest_route[3][0][1] - datetime.now()
    print(earliest_route[3][0])
    # totalTime += timedelta(seconds=walkingTime)
